...
class InterventionsService {
  ...
  Future<List<OptionItem>> getClientsMin() async {
    final token = (await StorageHelper.getUser())?['token'];
    final r = await _dio.get("lookup/clients/min",
  ...
  Future<List<OptionItem>> getTechniciensMin() async {
    final token = (await StorageHelper.getUser())?['token'];
    final r = await _dio.get("lookup/users/techniciens/min",
      options: Options(headers: { if (token!=null) 'Authorization':'Bearer $token' }),
    );
    return (r.data as List).map((e)=>OptionItem.fromJson(e)).toList();
  }
  ...
  Future<List<OptionItem>> getClientDiffuseursByClientMin(int clientId) async {
    final token = (await StorageHelper.getUser())?['token'];
    final r = await _dio.get("lookup/client-diffuseurs/by-client/$clientId/min",
      options: Options(headers: { if (token!=null) 'Authorization':'Bearer $token' }),
    );
    return (r.data as List).map((e)=>OptionItem.fromJson(e)).toList();
  }
  ...
  final payload = {
    "tafs": tafs.map((t) {
      ...
      if (cdId == null) {
        throw ArgumentError("clientDiffuseur.id est obligatoire pour chaque TAF");
      }
      return {
